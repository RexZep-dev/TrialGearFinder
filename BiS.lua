...
-- камень. «Уникальный использующийся» и одна категория «Алгарийский алмаз» —
-- на персонаже он ровно один. +12 в тултипе предмета — номинал, на двадцатке
-- оба дают +5 (таблица Харфа, «Камни»).
--
-- Ставим Кровавый камень выносливости: −5 % урона под контролем работает
-- в данжах, не только на арене (проверено пользователем 21 сентября).
-- Пометка PvP на камне — про источник и уникальную категорию, не про то,
-- что эффект мёртв в PvE. Кровавый камень разума по-прежнему не берём:
-- его выгрузка подставляла 12 сентября, это другой камень.
local PREFERRED_UNIQUE_PRIMARY = 213747 -- Кровавый камень выносливости

local function UniquePrimaryGem(primary)
    BuildGemIndex()
    local best, bestVal, bestPvp
    local prefID, prefVal
    for _, g in ipairs(ns.Gems or {}) do
        ...
        if g.socket == "prismatic" and g.unique and v then
            if g.itemID == PREFERRED_UNIQUE_PRIMARY then
                prefID, prefVal = g.itemID, v
            end
    ...
    if prefID then return prefID, prefVal end
    return best, bestVal
end
...
local function GemPicks(item, w, running, role)
    ...
    for i, t in ipairs(types) do
        ...
        elseif t == "meta" then
            ...
            if not picks[i] then
                -- Сначала камень «2 к основной + 2 к вторичке» — решение
                -- пользователя 12 сентября: так сборка ровнее по статам.
                -- Цена решения измерена симулятором и мала: на его сборке
                -- 1355 против 1372 у чистой основной, то есть 1.3%, а
                -- смешанный вариант вовсе оказался вровень (1370).
...
-- ---------------------------------------------------------------------------
-- Урон оружия. До 18 сентября окно сравнивало оружие ТОЛЬКО по статам, и это
-- было крупнейшей дырой: в симе замена двуручного на одноручное того же
-- уровня роняет урон Оружия с 392 до 218, хотя статы у одноручного не хуже.
-- Правильный выбор держался на ручных закреплениях в BiS_Data.
--
-- Считаем как игра: у оружия есть урон в секунду по уровню предмета, а приёмы
-- бьют от него, умноженного на НОРМАЛИЗОВАННУЮ скорость типа оружия. Отсюда
-- двуручное сильнее одноручного не в 1,32 раза (как по урону), а в 1,8.
--
-- Числа — из данных игры (ItemDamageOneHand.db2 / ItemDamageTwoHand.db2,
-- сборка 12.1.0). Для необычных, редких и эпических предметов значения
-- совпадают, поэтому качество не различаем. Проверено симулятором: 392
-- против 218 = 1,80, наша формула даёт 1,32 × 3,3/2,4 = 1,82.
local WEAPON_DPS = {
    [18] = { 2.93, 3.88 }, [19] = { 3.15, 4.17 }, [20] = { 3.37, 4.46 },
    [21] = { 3.45, 4.56 }, [22] = { 3.52, 4.66 }, [23] = { 3.60, 4.77 },
    [24] = { 3.68, 4.87 }, [25] = { 3.75, 4.96 }, [26] = { 3.81, 5.05 },
    [27] = { 3.88, 5.13 }, [28] = { 3.95, 5.23 }, [29] = { 4.02, 5.32 },
    [30] = { 4.09, 5.42 }, [31] = { 4.16, 5.52 }, [32] = { 4.24, 5.62 },
    [33] = { 4.32, 5.73 }, [34] = { 4.37, 5.78 },
}

-- Нормализованная скорость по типу оружия — правило игры, одно на все уровни.
-- Кинжал бьёт приёмами слабее одноручного меча при том же уроне в секунду:
-- 1,7 против 2,4. Ключ — subclassID из GetItemInfoInstant.
local WEAPON_NORM = {
    [0] = 2.4, [4] = 2.4, [7] = 2.4, [13] = 2.4, [15] = 1.7,  -- одноручные, кинжал
    [1] = 3.3, [5] = 3.3, [6] = 3.3, [8] = 3.3, [10] = 3.3,   -- двуручные
    [2] = 2.8, [3] = 2.8, [18] = 2.8,                         -- дальнобойные
}
local TWO_HANDED = { [1] = true, [5] = true, [6] = true, [8] = true, [10] = true }

-- Сколько «урона приёмами» даёт само оружие. nil — вещь не оружие.
local function WeaponPower(item)
    local id = item and item.itemID
    if not id then return nil end
    local _, _, _, _, _, classID, subclassID = C_Item.GetItemInfoInstant(id)
    if classID ~= 2 or not WEAPON_NORM[subclassID] then return nil end
    local lvl = item.ilvl or 23
    local row = WEAPON_DPS[lvl] or WEAPON_DPS[math.max(18, math.min(34, lvl))] or WEAPON_DPS[23]
    local dps = TWO_HANDED[subclassID] and row[2] or row[1]
    return dps * WEAPON_NORM[subclassID]
end
...
local function ScoreItem(item, w)
    ...
    -- Оружие: его урон дороже статов. Вес замерен симулятором (у Оружия
    -- единица урона в секунду стоит 10,12 против 1,70 у силы — в шесть раз).
    -- Спека нет в замерах — берём то же отношение к основной характеристике:
    -- это прикидка, но она несравнимо ближе к правде, чем ноль.
    --
    -- Кастерам прикидку не даём: они бьют заклинаниями, а не посохом, и вес
    -- урона оружия у них близок к нулю. Пока их не замерили, оружие у них
    -- сравнивается по статам, как и раньше. Признак — основная характеристика
    -- интеллект.
    local wp = WeaponPower(item)
    if wp then
        local bestPrimary, primaryKey = 0, nil
        for _, k in ipairs({ "str", "agi", "int" }) do
            if (w[k] or 0) > bestPrimary then bestPrimary, primaryKey = w[k], k end
        end
        local wdps = w.wdps or (primaryKey ~= "int" and bestPrimary * 6 or 0)
        s = s + wp * wdps
    end
