...
local ACTIONS = {
    ...
    [250] = { -- Кровь: Удар смерти, Удар в сердце, Вскипание крови, Дробление хребта.
              -- Штатная ротация SimC на двадцатке не жмёт ни одного приёма рыцаря:
              -- 24 урона, из них больше половины даёт аксессуар. Своя — 179
              -- (19 сентября, цель 45, сборка Арекстры с армори).
              -- Удар смерти в ротации обязателен: без него руническая сила
              -- переполняется и 93 % её пропадает впустую (154 против 179).
              -- Порог 35 — минимальная стоимость удара.
        "actions.precombat=raise_dead",
        "actions=auto_attack",
        "actions+=/death_strike,if=runic_power>=35",
        "actions+=/heart_strike",
        "actions+=/blood_boil",
        "actions+=/marrowrend",
    },
    [252] = { -- Нечестивость: чума до боя, Удар Плети, Гниющая мощь, Порча смерти.
              -- Штатная 161, своя 196 (19 сентября, цель 45, сборка Андарила).
              -- Состав сверен с логами гильдии: Удар Плети даёт там 61 %
              -- нажатий, у нас он главный источник урона.
              -- Льду своя ротация не нужна: штатная 118 против наших 77,
              -- и состав у неё совпадает с логами (Уничтожение, Ледяной удар).
        "actions.precombat=raise_dead",
        "actions=auto_attack",
    ...
    [253] = { -- Повелитель зверей: дерево двадцатки из логов, не Разрывающий.
              -- Разрывающий подменяет Верный выстрел, Кобра — Чародейский.
              -- С ними сим стоял 62 % без фокуса и давал 646. Живые жмут
              -- автовыстрел, Чародейский, Верный, Команду «Взять!», Гнев;
              -- Дикий трепет на пачке. С таким деревом 1052 (21 сентября,
              -- цель 45, лук Хеминга, +3 ловкости, 400 боёв), стояние 0 %.
              -- Середина логов 889, лучший 1339.
        "actions.precombat=summon_pet",
        "actions=auto_shot",
        "actions+=/bestial_wrath",
        "actions+=/kill_command",
        "actions+=/wild_thrash,if=active_enemies>=2",
        "actions+=/arcane_shot",
        "actions+=/steady_shot",
    },
    [255] = { -- Выживание: Обрез, бомба, Команда «Взять!», Удар ящера.
              -- Дерево как у Ускериа в логах (Обрез + Остриё копья), не Огнебомба.
              -- 485 на одной цели (21 сентября, цель 45, Жнец, +3 ловкости).
              -- Середина логов 974 — там пачки; на трёх целях с партизанским
              -- деревом 583. Обрез в симе ~3 раза за бой (кулдаун 60 с).
        "actions.precombat=summon_pet",
        "actions=auto_attack",
        "actions+=/boomstick",
